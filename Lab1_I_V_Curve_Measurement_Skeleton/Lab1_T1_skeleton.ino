...
void setup() {
  ...
  for (int onPeriod = 0; onPeriod < 256; onPeriod++) 
    ...
    analogWrite(Vin, onPeriod);// Output PWM with different duty cycle at Vin      
    ...
    x = float(onPeriod) * 100.0 / 255.0; // calculate duty cycle for TASK 1
    y = float(Vout_value) * 3.3 / 4095.0; // convert 12 bit binary number of Vout back to 0-3.3 V 
